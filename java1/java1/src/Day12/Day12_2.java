...
public class Day12_2 {
	...
	char sex = ssn.charAt(7); // 7번 인덱스의 문자를 추출
		...
		switch(sex){ // switch 제어문 [switch(검사대상)]
			case '1' : // sex 변수의 값이 1 이면
			case '2' : System.out.println("남자"); break; // sex 변수의 값이 2 이면
			case '3' : // sex 변수의 값이 3 이면
			case '4' : System.out.println("여자"); break; // sex 변수의 값이 14이면
		...
		// 1. 문자열 선언
		String strVar1 = new String("신민철"); // 문자열 == 불가능 [ == 힙 주소 비교]
		String strVar2 = "신민철"; // 객체는 new 연산자가 필수이지만 String클래스만 자동 객체 생성
		// 2. 문자열 비교
		if(strVar1 == strVar2) {
		...
		// p. 582 getBytes() : 문자열 -> 바이트열 변환
		
		String str = "안녕하세요"; // 영문 1바이트 한글 2바이트 >>> 5글자니까 10바이트
		// 문자열 -> 바이트열 변환
		byte[] bytes1 = str.getBytes(); // 문자열 -> 바이트열 변환
		System.out.println(bytes1.length); // 배열명.length()
		// 바이트열 -> 문자열 변환
		String str1 = new String(bytes1);
		System.out.println(str1);
		
		try {
			// 1. 인코딩타입 [인코딩/디코딩 : 변환방식]
		...
		// p. 504 intdexOf() : 문자 위치[인덱스] 찾기
		String subject = "자바 프로그래밍";
		// 문자열내 특정 문자열 위치[인덱스] 찾기
		...
		System.out.println("--------------------------------------------------------------");
			...
			String newStr = oldStr.replace("자바", "JAVA");
								// 문자열.replace("기본문자", "대치열문자")
			System.out.println("기존문자열 : " + oldStr);
			System.out.println("대치열문자열 : " + newStr);
			...
			String secondNum = ssn3.substring(7); // 인덱스 [~ 마지막인덱스전]
			System.out.println("확인 : " + secondNum);
			
			// split
			// "880815-1234567" "-" 기준 자르기시 2조각 -> 배열반환
			// [0] : 880815 [1] : 1234567
			System.out.println("일부분 : " + ssn3.split("-")[0]);
			System.out.println("윗부분 : " + ssn3.split("-")[1]);
		System.out.println("--------------------------------------------------------------");
			// p. 508
				// 영문문자열.toLowerCase -> 영문 [소문자로 변환]
				// 영문문자열.toUpperCase -> 영문 [eo문자로 변환]
			// p. 509
				// 문자열.trim() : 앞뒤 공백제거
			// p. 510 valueOf() : 기본타입 [int,double 등] -> 문자열 변환
			String str2 = String.valueOf(10); // int형 10 -> String 10 으로 변환
			String str3 = String.valueOf(10.6); // double형 10.5 -> Strint 10.5
			String str4 = String.valueOf(true); // boolean형 teue -> String true
			String str5 = 10+""; // int형을 String변수에 저장[불가능]
			
			System.out.println(str2 + 10); // 문자 + 문자 : 연결
			System.out.println(str3 + 10); // 문자 + 문자 : 연결
			// if(str4) 오류 : str5에는 voolean 형이 아닌 문자형 true 저장
