...
public class Day18_1 {
	...
	// p. 722 : 컬렉션 프레임워크
		// 여러개 객체를 추가, 검색, 삭제 -> 배열
		// 배열 문제점 : 
			// 1. 불특정 다수의 객체를 저장할 불가 [길이를 크게 설정 -> 비효율적]
			// 2. 삭제했을때 듬성듬성 빠진 옥수수 [삭제시 사이사이에 인덱스가 빈 현상]
		// 객체들을 효율[!]적으로 추가, 삭제, 검색 ---> 컬렉션프레임워크
		// 컬렉션 : 수집	프레임워크 : 사용방법을 미리 정해 놓은 라이브러리
		// * 저장(수집) 관련 클래스 / 인터페이스 포함
		// 1. List 인터페이스
			// 1. Arraylist 클래스
			// 2. Vector 클래스
			// 3. LinkedList 클래스
	...
	public static void main(String[] args) {
		// p. 728 : ArrayList 클랙스
			...
			// 다양한 메소드 제공
				// 1. 리스트객체명.add(객체명) : 해당 객체를 리스트 객체내 추가[비어있는 마지막 인덱스]
				// * 리스트객체명.add(인덱스, 객체명) : 해당 인덱스에 객체를 리스트 객체내 삽입 [기존 인덱스는 뒤로 인덱스 +1]
				...
				// 8. 리스트객체명.indexOf(객체명) : 리스트객체내 해당 객체의 인덱스 번호
		...
		ArrayList<String> list = new ArrayList<>();
			// 1. ArrayList : 리스트 클래스
				// 2. <제네릭> : 클래스 내부[리스트 내부]에 들어가는 자료형
					// 3. list : 리스트 객체명 (아무거나)
						// 4. new ArrayList<제네릭 = 생략가능>(길이 = 생량가능); // 생성자
		// 2. 리스트객체명.add(객체명 = 제네릭타입); 리스트객체 내 요소 추가
		list.add("java"); // 인덱스 0
		list.add("jdbc"); // 인덱스 1
		list.add("servlet/jsp"); // 인덱스 2
			System.out.println("리스트내 요소 확인 : " + list); // * 확인
		list.add(2, "Database"); // 인덱스 2에 요소추가 [기존 인덱스2 -> 인덱스3]
			System.out.println("특정 인덱스에 추가후 요소확인 : " + list);
		list.add("iBATIS");
		
		// 3. 리스트객체명.size() : 리스트객체 내 요소들의 개수 [길이]
		System.out.println("리스트내 요소들의 개수 : " + list.size());
		
		// 4. 리스트객체명.get(인덱스) : 리스트객체 내 특정 인덱스의 요소 호출
		System.out.println("리스트내 2번 인덱스 호출 : " + list.get(2));
		
		// 5. 반목문 활용 [리스트와 반복문 같이 사용] : 인덱스가 필요할때
		for(int i=0; i<list.size(); i++) { // i(인덱스)는 0부터 리스트 객체내 총 길이까지 1씩 증가반복
