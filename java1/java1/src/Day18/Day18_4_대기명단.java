...
public class Day18_4_대기명단 {
	...
	public static void main(String[] args) {
		...
		// 대기자입장 : 첫번째 대시중인 대기번호 호출
		Scanner scanner = new Scanner(System.in);

		int 대기번호 = 1; // 오늘의 대기번호
		ArrayList<대기> 대기명단 = new ArrayList<>();
		// 여러개의 대기 객체를 저장할 수 있는 리스트 객체 선언
		...
		while (true) {
			...
			if (ch == 1) {
				...
				// * 대기명단 리스트에 하나씩 temp 호출하는데 만약에 temp내 대기번호가 입력받은 삭제 대기번화 같으면 해당 temp는 삭제후
				// break;

			} else if (ch == 3) {

				System.out.println("가장 앞에 있는 대기명단을 입장 시킵니다. ");
				대기명단.remove(0); // 0번째 인덱스 삭제
