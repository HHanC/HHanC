...
public class Day19_1 {
	...
	public static void main(String[] args) {
		...
		// 컬렉션(수집=저장)프레임워크(미리만들어진클래스)
			...
			// 1. List 컬렉션				
				// * 인덱스(저장순서)
				// 1. ArrayList 클래스	: 싱글스레드 / 순차적 추가, 삭제 비교적 느림
		...
		set.add("java");			System.out.println("확인(중복불가능) : " + set); // 객체 중복 불가능
		...
		// 4. Iterator : 저장된 객체를 한번씩 가져오는 반복자 리턴 (순회)
			// * set컬랙션 에서 사용되는 이유 : 인덱스가 없기 때문에 객체를 호출하는 방법x get
		Iterator<String> iterator = set.iterator(); // set객체네 iterator() : 순회
		...
		// * for문과 컬렉션 활용
			// for(임시 객체명 : 컬렉션/배열) : 컬렉션/배열내 요소들을 하나씩 가져오는 반복자
		...
		// 6. set객체명.clear() : set객체네 모든 객체를 비우기[삭제]
