	...
	public static void main(String[] args) {
		...
		// 1. Map 컬렉션
			...
			// HashMap 클래스
				...
				// 5. map객체명.values() : amp객체내 모든 값 호출
...
//HashMap : 컬렉션프레임워크 map 클래스
		...
		map.put("동장균", 80); System.out.println("확인 : " + map);
		// * key는 중복 불가능 [기존 key 없어지고 새로운 키로 대치] 
		map.put("홍길동", 95); System.out.println("확인 : " + map);
		// * value는 중복가능
		map.put("유재석", 90); System.out.println("확인 : " + map);
		// 3. map객체명.size() : 엔트리(키=값) 개수
		System.out.println("map 객체내 엔트리 수 : " + map.size());
		// 4. map객체명.get(키) : map객체내 키 -> 값 호출
		System.out.println("map 객체내 값 호출(키 호출시 -> 값 호출) : " + map.get("유재석"));
		...
		// 7. 반목문 [key를 반복문 이용해서 모든 value 호출 가능]
