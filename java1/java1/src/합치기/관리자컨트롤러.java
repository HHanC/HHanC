...
public class 관리자컨트롤러 {
	...
	void save() {
		try {
			FileOutputStream OutputStream = new FileOutputStream("D:/자바/영화저장.txt");
			for( 관리자_클래스 temp : 영화저장 ) {
	...
	static void load() {
		try {
			FileInputStream inputStream = new FileInputStream("D:/자바/영화저장.txt");
			...
			int i = 0; 
			for( 관리자_클래스 temp : 영화저장 ) {
				if( i+1 == 영화저장.size() ) break;
				String[] field = temp.toString().split(",");
				관리자_클래스 음식 = new 관리자_클래스( field[0], (field[1]));  
				영화저장.add(음식);
				i++; 
			}
