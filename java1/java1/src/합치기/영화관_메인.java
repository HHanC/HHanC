...
//				boolean 통과 = false;
//				for(int j = 0; j<관리자컨트롤러.영화출력.size(); j++) {
//					for(int i=0; i<관리자컨트롤러.영화출력.size(); i++) {
//						if(관리자컨트롤러.영화출력.get(i) != null && 관리자컨트롤러.영화출력.get(i).get영화제목() == 관리자컨트롤러.영화출력.get(j).get영화제목()) {
//							System.out.println(관리자컨트롤러.영화출력.get(i).get영화제목());
//						}else {
//							System.out.println(관리자컨트롤러.영화출력.get(i).get영화제목());
//							System.out.println(관리자컨트롤러.영화출력.get(j).get영화제목());
//						}
//					}
//				}
			...
			if(ch==1){
				...
				if(result) {
					...
					for(합치기.관리자_클래스 temp : 합치기.관리자컨트롤러.영화저장) {
						if(temp != null && temp.get영화제목().equals(movieTitle)) {
							System.out.print("영화시간 : "+temp.get영화시간());
							System.out.println();
						}							
					}
					System.out.println("----------------------------------------------------");
