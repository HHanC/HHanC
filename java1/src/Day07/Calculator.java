...
public class Calculator {
	...
	// 2. 생성자
		...
		// 기본생성자 : 매개변수(인수)가 없다.
	// 3. 메소드
	void powerOn() { // 1. 인수가x반환x
		System.out.println("전원을 켭니다.");
		
	}
	// 1. void : 리턴타입
		// void 함수가 종료되면서 나가는 테이터가 없다.
			// return; 생략 가능[ 단 실행부 중간에 사용시 함수종료 ]
	// 2. powenOn : 메소드이름[아무거나]
	...
	// 1. int : 리턴타입 [ 함수가 종료되면서 int형 데이터가 반환 ]
		// 5. return 반환값;
			// 리턴타입이 int 이기떄문에 return 에는 무조건 int형 데이터 
	// 2. plus : 메소드이름[아무거나]
	// 3. (int x, int y) : int형 매개변수 받기
		// 함수호출시 함수로 들어오는 xㅘ y는 int형으로 받기
	// 4. {실행부}
	
	double divide(int x, int y) { // 인수o반환o
//	리턴타입	메소드명(인수 1, 인수2) { 실행코드; return }
		double result = (double)x / (double)y;
				// (자료형)변수명 : 강제형변환
					// int형변수 -> double 변경
						// 왜?? 나누기후에 소수점 나올 수 있기 때문에 double로 강제형 변환
