...
//				private : 현재 클래스내에서 사용가능
					// 목적 : 필드 직접접근 제한 [필드에 저장 / 호출시 데이터 검사(무결성 유지)]
//				public	: 모든 클래스 내에서 사용가능
//				default(생략시) : 동일한 패키지[폴더]내 사용가능
//				protected : 동일한 패키지[폴더]내 사용가능 ( *단 : 서로 다른 패키지의 슈퍼클래스 접근가능)
			// * : static, final = 상수
			// 2. 자료형(클래스, 기본자료형)
				// 1. 기본 자료형 [int, double, char, boolean 등]
				// 2. 클래스 [사용자 만든 클래스] : 회원클래스, 게시판클래스, 제품클래스, 카클래스
				// 3. 클래스 [미리 만든 클래스] : API (String, Scanner, ArrayList 등)
				// 4. 인터페이스
				// 5. [ ] 배열
			// 3. 변수명(객체명)
				// * 본인이 원하는대로 하되 중복이름x
				// 1. 클래스명의 소문자[Scanner scanner, Date date](관례적으로)
	
			// * 변수, 객체(new), 인터페이스, 배
		// 2. 생성자 [객체 생성시 초기값 = 처음값]
			// 1. 접근제한자
			// 2. 생성자이름 = 클래스이름 [만일 다른경우 오류]
			// 3. 인수[매개변수]
				// * 필요에 따른 인수 선택
