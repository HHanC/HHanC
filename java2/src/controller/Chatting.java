...
public class Chatting implements Initializable {
    ...
    public void show() {
    	...
    	ObservableList<Room> roomlist = RoomDao.roomDao.roomlist(); 			// 1. DB에서 모든 방 목록 가져오기
    	TableColumn tc = roomtable.getColumns().get(0); 						// 2. 테이블뷰의 첫번째 필드를 가져와서 방번호 필드로 설정 
    	tc.setCellValueFactory( new PropertyValueFactory<>("ronum"));
    	tc = roomtable.getColumns().get(1); 									// 3. 테이블뷰의 두번째 필드를 가져와서 방이름 필드로 설정 
    	tc.setCellValueFactory( new PropertyValueFactory<>("roname"));
    	tc = roomtable.getColumns().get(2);										// 4. 테이블뷰의 세번째 필드를 가져와서 방인원 필드로 설정
    	tc.setCellValueFactory( new PropertyValueFactory<>("mcount"));
    	roomtable.setItems( roomlist ); 										// 5. 테이블뷰에 리스트를 넣어주기  
    	roomtable.setOnMouseClicked( e -> {  									// 6. 해당 테이블뷰를 클릭했을때.
    		selectroom =  roomtable.getSelectionModel().getSelectedItem(); 		// 7. 클릭된 객체(방) 가져와서 객체(방)에 저장
    		lblselect.setText("현재 선택된 방 : "+selectroom.getRoname() );		// 8. 레이블 표시 방 이름 표시
    		btnconnect.setDisable(false); 										// 9. 접속 버튼 사용 활성화
    	} );
    }
    ...
    public void midshow() { // msg메소드 : 입력창에서 엔터 쳤을때 // send메소드 : 전송버튼 눌렀을때
    	ArrayList<Roomlive> roomlivelist = RoomDao.roomDao.getRoomlivelist( selectroom.getRonum() ); // 1. 테이블 뷰에서 선택된 방 번호의 접속된 회원명단 가져오기
