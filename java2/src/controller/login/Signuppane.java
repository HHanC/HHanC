...
public class Signuppane implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		lblconfirm.setText(""); // 처음 실행 문구 가림
	}
    ...
    void signup(ActionEvent event) {
    	...
    	// * 유효성검사[ 제한 ] 
    		...
    		if(id.length() < 4 || id.length() > 10) {
    			lblconfirm.setText("[실패] 아이디는 4~10사이로 입력해 주세요!");
    			return; // 가입 못하게 메소드 종료
    		}
    		// 3. 비밀번호 길이 체크
    		if(password.length() < 4 || password.length() > 10) {
    			lblconfirm.setText("[실패] 패스워드는 4~10사이로 입력해 주세요!");
    			return; // 가입 못하게 메소드 종료
    		}
    		// 4. 비밀번호 일치 체크
    		...
    		// 5. 이메일 체크 [문자열.mail.indexOf("문자") : 해당 문자열내 문자가 존재하면 해당 문자의 인덱스 호출/ 만일 없으면 -1]
