...
public class Productcontrol implements Initializable {
    ...
    @FXML
    void serch(ActionEvent event) { // 검색 버튼 눌렀을때
    	
    	String serch = txtserch.getText(); // 검색창에 입력된 데이터 가져오기
    	show(serch); // 입력한 검색어를 show 메소드에 넣어주기
    }
    
    void show(String serch) {
    	
    	if(vbox.getChildren().isEmpty() == false) { // .isEmpty() : 해당 객체내 비엉있는지 확인[vbox 내 비어있는지 확인]
    		// vbox내 객체가 비어있지 않으면
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		show(null); // 화면을 처음 열렸을떄는 검색을 null 넣어주기
	}
}
