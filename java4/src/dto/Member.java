...
public class Member { // 데이터 모델
	...
	public static void sendmail(String 받는사람이메일, String 내용) {
		...
		// 2. 호스트 설정
		Properties properties = new Properties(); // 컬렉션프레임워크[map]
		properties.put("mail.smtp.host", "smtp.naver.com"); // 호스트 주소
		properties.put("mail.smtp.prot", 587); // 호스트 포트번호
		properties.put("mail.smtp.auth", "true"); // 보내는사람 이메일 인증
		properties.put("mail.smtp.ssl.protocols", "TLSv1.2"); // 보안 연결 버전 설정
		// 3. 인증처리 [Seesion : javax.mail 패키지]
			// Session.getDefaultInstance(설정객체, 인증객체)
		Session session = Session.getDefaultInstance(properties, new Authenticator() {
			
			@Override // 보내는 사람의 이메일주소, 비밀번호 인증 해주는 메소드 구현
		...
		try {
			...
			// 내용
			message.setSubject("안산지역 중고거래 회원님의 패스워드 찾기");
