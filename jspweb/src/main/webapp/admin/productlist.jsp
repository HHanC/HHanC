...
<body>
		...
		%>
			<tr>
				...
				<th> <!-- 색상과 사이즈에 따른 재고량 표시  -->
					...
					<% }%>
					<%}else{ // 재고리스트가 비어 있으면 %>
						<span id='amountbox<%=p.getPno()%>'> - </span> 
					<%} %>
				</th> 			
				...
				<th> 
					...
					<button onclick="getstock(<%=p.getPno() %>)" data-bs-toggle="modal" data-bs-target="#stockmodal" class="">재고 변경</button> 
				</th>
			</tr>
		<%
	...
	<div class="modal" tabindex="-1" id="stockmodal">
	  <div class="modal-dialog">
	    <div class="modal-content">
	      <div class="modal-header">  <!-- 모달 제목 -->
	        <h5 class="modal-title"> 제품의 재고 변경 </h5>
	       		<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
	      </div>
	      <div class="modal-body">  <!-- 모달 내용  -->
	      ...
	      <div class="modal-footer"> <!-- 모달 버튼 -->
	      		<button type="button" class="btn btn-primary" onclick="stockupdate(3)">수정 처리</button>
