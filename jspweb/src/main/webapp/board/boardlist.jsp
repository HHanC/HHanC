...
<!-- -------------------------------------------페이징 계산 처리---------------------------------- -->
	...
	<%
		...
		if(key != null && keyword != null){ // 검색이 있을경우 
			// 검색 내용 기록
...
<!-- -------------------------------------------페이징 계산 처리---------------------------------- -->
	<%
	// 1. 게시물 전체 개수
		int totalrow = BoardDao.getBoardeDao().gettotalrow(key, keyword);
		// 2. 현재 페이지번호 
		int currentpage = 1;
			// *요청받은 페이지번호 
		...
		ArrayList<Board> boardlist 
			= BoardDao.getBoardeDao().getboardlist( startrow , listsize , key , keyword );
...
<!-- ------------------------테이블 구역---------------------------- -->
		<table class="table table_hover text-center"> <!-- 테이블 -->
			<tr>
				<th width="10%"> 번호 </th>
				<th width="50%"> 제목 </th> 
				<th width="15%"> 작성자 </th> 
			...
			<%
				...
				int count = BoardDao.getBoardeDao().replycount(board.getBno());
			%>
				<!-- 
					행을 클릭했을때[ js ]   
