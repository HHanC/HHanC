...
<%@page import="controller.product.cancelorder"%>
...
<body>
	...
	<div class="container">
		<%
		// * 공공데이터 포털
			// 1. url 가져오기 [url : java.net패키지 -> java변환]
			URL url = new URL("https://api.odcloud.kr/api/3035882/v1/uddi:5fae3cf5-bc15-4eba-87d8-8289b74e659b_201912202015?page=1&perPage=300&serviceKey=a2WfEP1Rh%2F8Gkfnevq2N8WNRiv5VGZSLi1VzuN0sH%2B%2FN519tfndNpJnHNA73vXk3UZyAhwqnpV3TMtcs3YTAgQ%3D%3D");
...
/* 		LocalTime localTime2 = LocalTime.now();
		System.out.println(localTime2); */
			...
			for(int i = 0; i<array.length(); i++){
				...
				if(keyword != null){ // 만약에 검색이 있을경우
					...
					if(index == -1){continue;} // -1 인덱스 없다는 의미
					// for문, while문 : 1.break : 반복문 종료 , 2. continue : 반복문 재실행
				}
				...
				while(iterator.hasNext()){ // hasNext() : 다음 객체가 있는지 확인
					...
					if(key.equals(현재요일 + "요일 운영")){  // 현재 요일과 동일한 키만 호출
						...
						jo.put("영업여부", jo.get(key)); // i번째 json객체에 새로운 엔트리 추가 [키 : 값] 추가
					}
				} */
				// JSONObject.getNames(json 객체) : 해당 json객체내 모든 key 호출
				...
				for(String key : keys){
					...
					if(key.equals(현재요일 + "요일 운영") && !jo.getString(key).equals("-")){
						...
						if(Integer.parseInt(영업시간[1].split(":")[0]) <= 24  ){
							...
							if(현재시간.isAfter(여는시간)){ // 현재시간이 여는 시간보다 이후이면
	...
	<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=a66203c278743e8b1eafd1a178999699&libraries=services,clusterer,drawing"></script>
	<!-- kakao 지도 -->
	<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=a66203c278743e8b1eafd1a178999699"></script>
