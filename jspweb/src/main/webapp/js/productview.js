...
$("#color_select").change( function(){
	...
	// js 배열 
		...
		// 특정인덱스 삭제 : splice( 시작인덱스 , 개수 ) -> 특정 시작인덱스부터 개수만큼 삭제 
			// 배열내 중간 인덱스 삭제시 빈공간 당기기 -> java 리스트 클래스 동일
				// 예) 3번 인덱스 삭제시 : 4번 인덱스 -> 3번 인덱스 당겨짐
	// js 객체 선언 -> object = {   }
...
$("#size_select").change( function(){
	...
	/* 천단위 구분 쉼표 -> 정규 표현식(언어) vs 
			js(내장 메소드 : toLocaleString())	
				데이터.toLocaleString(undefinde, {maximumFracitonDigits : 소수점 표시단위})
	...
	*/
		...
		/* 
			...
			\d{3} : 정수 3자리 패턴
			(\d{3}) : 앞 표현식 반복 대응
			(\d{3})+(?!\d) : 앞 표현식 뒤에 정수가 없는 경우 [숫자 끝 찾기]
			...
			{} : 길이
			$/ : 정규표현식 끝
			+ : 앞 표현식 반복되는 부분 대응(천단위, 백단위)
			x(?!y) : x 뒤에 y가 없는경우(false) : (?!\d) : 앞에 패턴이 없는 경우 (뒤에 숫자가 없는경우)
...
function saveplike( mid ){
	if(mid == 'null'){alert("로그인 후 등록 가능합니다.");  return;}
	let pno = $("#pno").val();
	alert("!@3");
	$.ajax({
		url : "saveplike" ,
		...
		success : function(re){
			if(re == 1){alert("관심등록 했습니다.");}
			else if(re == 2){alert("관심등록을 취소 했습니다.");}
			else if(re == 3){alert("오류발생. 관리자문의");}
			// 특정 태그 새로고침
...
function savecart(mno){
	// 로그인이 안 되어 있으면
	if(mno == 0){alert("로그인 후 장바구니 사용 가능합니다."); return;}
	// 선택한 옵션의 개수 -> 만약에 선택한 옵션이 없으면
	if(selectlist.length == 0) {alert("최소 하나의 옵션 선택해 주세요"); return;}
	
	// 서블릿에게 배열 보내기 (JSON : 서로 다른 언어들끼리 데이터 타입 통일성)
		// ajax <-----json------> 서블릿
			// js : json 라이브러리 내장 // java : json 라이브러리 다운로드
			// JSON.stringify( ) : 배열이나 객체를 JSON형식으로 반환 메소드
			// key : value -> entry
			// 여러개 entry -> JSONobject
			// 여러개 JSONobject -> JSONarray
			// 키 : value(JSONarray))
			// {키1 : 값 , 키2 : 값 , 키3 : 값}
